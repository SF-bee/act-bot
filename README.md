...
| 活跃统计 | `/stats` | 消息数 / 活跃人数 / 入群退群 / 在线时长，**成员与管理员看到的内容不同**（管理员多看命令数、自动回复数、错误数、今日活跃 Top N、近 N 天汇总）；内存累加 + 定时落库（`[stats].flush_seconds`），**不存消息正文、不落库每人发言数**；开关走 `/config stats on|off` |
